...
# LSTM Predictor (اختياري)
_bybit_lstm_predictor = None
try:
    from core.lstm_predictor import LSTMPredictor as _BybitLSTMCls
    _bybit_lstm_predictor = _BybitLSTMCls(model_dir='/root/trade_lak_bot/models')
    if _bybit_lstm_predictor.is_active:
        logging.getLogger('bybit_scanner').info(f"[BybitScanner] ✅ LSTM محمَّل | features={_bybit_lstm_predictor.n_features}")
    else:
        _bybit_lstm_predictor = None
except Exception as _e_lstm:
    logging.getLogger('bybit_scanner').debug(f"[BybitScanner] LSTM غير متاح: {_e_lstm}")
    _bybit_lstm_predictor = None
...
class BybitScanner:
    ...
    def _analyze_symbol(self, symbol: str) -> Optional[Dict]:
        ...
        try:
            ...
            # === LSTM Score Boost ===
            if _bybit_lstm_predictor and _bybit_lstm_predictor.is_active:
                try:
                    # تحويل klines إلى تنسيق OHLCV
                    _ohlcv_bybit = [[k[0], float(k[1]), float(k[2]), float(k[3]), float(k[4]), float(k[5])] for k in reversed(klines)]
                    score, _lstm_r = _bybit_lstm_predictor.boost_score(score, _ohlcv_bybit, {})
                    if _lstm_r.get('active'):
                        _ld = _lstm_r.get('direction', '?')
                        _lp = _lstm_r.get('lstm_prob', 0)
                        signals.append(f"LSTM:{_ld}({_lp:.2f})")
                        logger.debug(f"[BybitScanner][LSTM] {symbol}: {_ld} prob={_lp:.2f} score={score:.2f}")
                except Exception as _le:
                    logger.debug(f"[BybitScanner][LSTM] {symbol}: {_le}")
            # === نهاية LSTM ===
            return {
                'symbol': symbol,
                'price': price,
    ...
    def execute_bybit_trade(self, symbol: str, price: float, score: float) -> bool:
        ...
        try:
            ...
            trade_size = round(min(usdt_balance * 0.25, 50.0), 2)
