...
def check_services() -> list:
    ...
    for svc in SERVICES:
        try:
            ...
            # عند timeout: تحقق عبر طريقة بديلة
            try:
                r2 = subprocess.run(
                    ['systemctl', 'show', svc, '--property=ActiveState'],
                    capture_output=True, text=True, timeout=15
                )
                if 'ActiveState=active' in r2.stdout:
                    logger.info(f"✅ خدمة {svc}: active (fallback check)")
                    _clear_alert(f'svc_{svc}')
                else:
                    issues.append((svc, f'خطأ: {e}'))
            except Exception as e2:
                issues.append((svc, f'خطأ: {e2}'))
    return issues
...
def check_flash_sniper_log() -> tuple:
    try:
        ...
        status = result2.stdout.strip()
        if status == 'active':
            logger.info("✅ Flash Crash Sniper: active (retry)")
            _clear_alert('flash_sniper_log')
            return True, None
        ...
        try:
            r2 = subprocess.run(
                ['systemctl', 'show', 'flash-crash-sniper', '--property=ActiveState'],
                capture_output=True, text=True, timeout=15
            )
            if 'ActiveState=active' in r2.stdout:
                logger.info("✅ Flash Crash Sniper: active (fallback)")
                _clear_alert('flash_sniper_log')
                return True, None
        except Exception:
            pass
