...
def main():
    ...
    # 3. جلب بيانات CoinGlass
    print("💎 جلب بيانات CoinGlass...")
    coinglass_data = {}
    ...
    for sym in ["BTC", "ETH", "SOL", "XRP", "BNB", "DOGE"]:
        try:
            # Funding Rate
            r = requests.get(f"{cg_base}/api/futures/funding-rate/current",
                           params={"symbol": sym}, headers=cg_headers, timeout=10)
            if r.status_code == 200:
                data = r.json()
                if str(data.get("code")) == "0":
                    items = data.get("data", [])
                    if items:
                        rates = [float(x.get("fundingRate", 0)) for x in items if x.get("fundingRate")]
                        avg_rate = sum(rates) / len(rates) if rates else 0
                        coinglass_data.setdefault(sym, {})["funding_rate"] = avg_rate
        except:
            pass

        try:
            # Liquidations
            r = requests.get(f"{cg_base}/api/futures/liquidation/v2/history",
                           params={"symbol": sym, "interval": "1h"}, headers=cg_headers, timeout=10)
            if r.status_code == 200:
                data = r.json()
                if str(data.get("code")) == "0":
                    items = data.get("data", [])
                    if items:
                        ...
                        coinglass_data.setdefault(sym, {})["long_liq"] = float(latest.get("longLiquidationUsd", 0))
                        coinglass_data.setdefault(sym, {})["short_liq"] = float(latest.get("shortLiquidationUsd", 0))
        except:
            pass
    ...
    top = [o for o in opportunities if o["success_rate"] >= 85][:5]

    if not top:
        # لا توجد فرص 85%+ - صمت ولا نرسل شيئاً
