...
**قواعد المخاطر الكلية**: Max 15 Spot | Max Drawdown يومي 8% → إيقاف
...
```
/root/trade_lak_bot/
├── main.py                    ← الكلاس الرئيسي TradeLakBotV4
├── startup_health_check.py    ← فحص صحي عند كل تشغيل (15 نقطة)
├── sector_liquidity_hunter.py ← محرك القطاعات
├── fake_break_detector.py     ← محرك Fake Break
├── post_entry_monitor.py      ← مراقبة ما بعد الدخول
├── core/
│   ├── intelligence_engine.py ← محرك القرار الرئيسي
│   ├── strategy.py            ← منطق الخروج
│   ├── market_scanner.py      ← فحص الفرص
│   └── advanced_risk_manager.py
└── config/config.py           ← الإعدادات (MAX_SPOT_TRADES=15)
```

```bash
# فحص حالة Trade Lak
ssh root@164.68.112.131 "tail -50 /root/trade_lak_bot/logs/bot.log"
# إعادة التشغيل
ssh root@164.68.112.131 "pm2 restart trade_lak"
# أو
ssh root@164.68.112.131 "cd /root/trade_lak_bot && nohup python3 main.py &"
```
...
8. **الحد الأدنى OKX**: $10 لكل صفقة (MIN_ORDER_USDT = 10)
9. **MAX_SPOT_TRADES**: يجب أن يكون ≥ عدد العملات في المحفظة (الإعداد الحالي: 15)

---

## 11. الدروس المستفادة (مشاكل حقيقية وحلولها)

هذا القسم يوثق المشاكل الفعلية التي ظهرت خلال تشغيل Trade Lak وكيفية حلها. كل مشكلة هنا يفحصها `startup_health_check.py` تلقائياً عند كل إعادة تشغيل.

### جدول المشاكل الموثقة

| # | المشكلة | السبب | الحل | الفحص التلقائي |
|---|---------|-------|------|----------------|
| 1 | MAX_SPOT_TRADES يمنع الفرص | الحد كان 3 أو 5 | رفع إلى 15 | يتحقق أن القيمة ≥ 10 |
| 2 | المزامنة تُحذف بعد التشغيل الأول | ملف مؤقت يُعاد تسميته | استبدال بـ `_auto_sync_portfolio()` | يتحقق من عدم وجود ملفات .pkl قديمة |
| 3 | رصيد وهمي يوقف Trade Lak | `usdt_free` فقط دون العملات | فحص الصفقات المفتوحة أيضاً | يتحقق من الرصيد مع الأخذ بعين الاعتبار الصفقات المفتوحة |
| 4 | direction خاطئ في trade_record | `BUY` بدلاً من `SPOT_BUY` | تصحيح القيمة | يُصحح تلقائياً لجميع الصفقات |
| 5 | amount_coin مفقود | نسيان إضافته في المزامنة | إضافة `amount_coin: qty` | يجلب من OKX ويُضيف تلقائياً |
| 6 | open_time مفقود | نسيان إضافته في المزامنة | إضافة `open_time: datetime.now()` | يُضيف تلقائياً لكل صفقة ناقصة |
| 7 | Telegram HTML parsing errors | أحرف `< > &` غير مُعالَجة | `html.escape()` على النصوص الديناميكية | يُرسل رسالة اختبار HTML عند التشغيل |
| 8 | FakeBreakDetector يكتشف عند المقاومة فقط | السوق بعيد عن الدعم | سلوك صحيح — انتظار الدعم | يتحقق من تهيئة المحرك |
| 9 | عملات صغيرة تُتجاهل | حد التجاهل كان $5 | تخفيض الحد إلى $1 | يتحقق من تغطية جميع العملات > $1 |
| 10 | OKX يرفض الصفقة | المبلغ أقل من $10 | `MIN_ORDER_USDT = 10` | يتحقق أن الحد ≥ 10 |
| 11 | Trade Lak يتوقف عند 3 صفقات | إعداد محافظ قديم | رفع MAX_SPOT_TRADES إلى 15 | يتحقق أن القيمة ≥ 10 |

### كيفية إضافة مشكلة جديدة

عند ظهور مشكلة جديدة، اتبع هذه الخطوات:

**الخطوة 1:** وثّق المشكلة في `known_issues.md` بالتنسيق:
```
### N. اسم المشكلة
- المشكلة: وصف ما حدث
- السبب: لماذا حدث
- الحل: ما الذي طُبِّق
- الفحص عند التشغيل: كيف نتحقق منه
```

**الخطوة 2:** أضف فحصاً جديداً في `startup_health_check.py`:
```python
def _check_new_issue(self):
    """#N وصف المشكلة"""
    # منطق الفحص
    if problem_detected:
        # إصلاح تلقائي إن أمكن
        return False, "وصف المشكلة", True  # True = تم الإصلاح تلقائياً
    return True, "كل شيء طبيعي ✓", False
```

**الخطوة 3:** أضف الفحص لقائمة `checks` في دالة `run()`.

**الخطوة 4:** حدّث هذا الجدول في SKILL.md.

---

## 12. نظام Self-Healing Checklist

### المبدأ

كل مشكلة تظهر مرة واحدة يجب أن تُحوَّل إلى فحص تلقائي يمنع تكرارها. هذا هو جوهر نظام Self-Healing في Trade Lak.

### الملف: `startup_health_check.py`

يُشغَّل تلقائياً في نهاية `__init__` من `TradeLakBotV4` عبر:
```python
...
| # | الفحص | نوعه | يُصلح تلقائياً؟ |
|---|-------|------|----------------|
| 1 | اتصال OKX | حرج | لا (يوقف التشغيل) |
| 2 | ملف الإعدادات موجود | حرج | لا |
| 3 | MAX_SPOT_TRADES كافٍ | تحذير | نعم (يرفع إلى 15) |
| 4 | مزامنة المحفظة | تحذير | نعم (يُشغّل المزامنة) |
| 5 | direction صحيح في الصفقات | تحذير | نعم (يُصحح إلى SPOT_BUY) |
| 6 | amount_coin موجود | تحذير | نعم (يجلب من OKX) |
| 7 | open_time موجود | تحذير | نعم (يضع datetime.now()) |
| 8 | MIN_ORDER_USDT ≥ 10 | تحذير | نعم (يرفع إلى 10) |
| 9 | Telegram يعمل | تحذير | لا |
| 10 | FakeBreakDetector مُهيَّأ | تحذير | لا |
| 11 | PostEntryMonitor نشط | تحذير | لا |
| 12 | SectorLiquidityHunter متاح | تحذير | لا |
| 13 | لا ملفات مزامنة قديمة | تحذير | نعم (يحذفها) |
| 14 | منطق الرصيد سليم | تحذير | لا |
| 15 | جميع العملات مراقبة | معلومة | لا |

### تقرير Telegram عند التشغيل

عند كل إعادة تشغيل، يُرسل Trade Lak تقريراً عبر Telegram يتضمن:
- عدد الفحوصات الناجحة / الإجمالية
- الإصلاحات التلقائية التي طُبِّقت
- المشاكل الحرجة إن وُجدت
- إحصائيات المحفظة الحالية
